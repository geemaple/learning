...
if __name__ == '__main__':
    ...
    drawdendrogram(cluster,labels=blognames,jpeg='hcluster.jpg')
    coords=scaledown(data)
    draw2d(coords,blognames,jpeg='blogs2d.jpg')

    print 'hierarchy cluster take %s' % (time.time() - start)

    start = time.time()
    cluster = kcluster(data, k=10)
    print cluster
    print 'k-mean cluster take %s' % (time.time() - start)

    # start = time.time()
    # cluster = kcluster(data, k=20)
    # print cluster
    # print 'k-mean cluster take %s' % (time.time() - start)
    #
    # start = time.time()
    # cluster = kcluster(data, k=50)
    # print cluster
    # print 'k-mean cluster take %s' % (time.time() - start)

    (items, users, data) = readfile('zebo.txt')
    start = time.time()
    cluster = hcluster(data, distance=tanamoto)
    drawdendrogram(cluster, labels=items,jpeg='zeboCluster.jpg')
    print 'hierarchy cluster take %s' % (time.time() - start)
